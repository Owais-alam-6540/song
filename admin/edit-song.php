...
$S_id = $_GET['id'];
$fetch = "SELECT * FROM `song` WHERE `Song_Id` = '{$S_id}'";
...
if(isset($_POST['btn'])){
   ...
   $song_thumb = $thumbnail['name'];
   $tmp1 = $thumbnail['tmp_name'];

   $update = "UPDATE `song` SET `Songs` = '{$gana}' , `Artist`	 = '{$artistname}' , `Language` = '{$songlang}' , `Genre` = '{$songgen}' , `Release_date` = '$releasedate' , `thumbnail` = '$song_thumb' WHERE `Song_Id` = '{$id}' ";
   $updateResult = mysqli_query($conn, $update);
   if ($updateResult) {
      move_uploaded_file($tmp, "../songs/$gana");
      move_uploaded_file($tmp1, "../thumbnails/$song_thumb");
      echo"
      <script>
       setTimeout(()=>{
                  ...
                  <div class="iq-card">
                     ...
                     <div class="iq-card-body">
                        <form  method="post" enctype="multipart/form-data">
                          <?php 
                          while(
                           $row4 = mysqli_fetch_array($fetchResult)){
                          ?>
                          <input type="hidden" class="form-control" name="hidden" value='<?php echo $row4['Song_Id']?>'>
                          <div class="form-group">
                              <label>Song Name:</label>
                              <div class="custom-file">
                                 <input type="text" class="form-control" placeholder="Song name"  value='<?php echo $row4['Song_Name']?>'  name="songname" required>
                              </div>
                           </div>
                           <div class="form-group">
                           ...
                           <div class="form-group">
                              <label> Update Songs:</label>
                              <div class="custom-file">
                                 <input type="file" class="custom-file-input" id="customFile" name="songs" value='<?php echo $row4['Songs']?>' required>
                                 <label class="custom-file-label" for="customFile">Choose file</label>
                              </div>
                           </div>
                           <div class="form-group">
                              <label>Artist Name:</label>
                              <select class="form-control" id="exampleFormControlSelect1" name="aname"  value='<?php echo $row4['Artist']?>' required>
                           ...
                           <div class="form-group">
                              ...
                              <select class="form-control" id="exampleFormControlSelect1" name="sonlang" required>
                                 ...
                                 while($row = mysqli_fetch_array($result)){
                                    if($row[0]==$row4["Language"]){
                                       $sele1="Selected";
                                    }else{
                                       $sele1="";
                                    }               
                                 ?>
                                 <option <?php echo $sele1?> value="<?php echo $row [0]?>"><?php echo $row [1]?></option>
                                 <?php  }
                                 ?>
                                 </select>
                           </div>
                           <div class="form-group">
                              ...
                              <select class="form-control" id="exampleFormControlSelect1" name="songen" required>
                                 ...
                                 while($row2 = mysqli_fetch_array($result1)){    
                                    if($row2[0]==$row4['Genre']) {
                                       $selec = "selected";
                                    } else{
                                       $selec = "";
                                    }                               
                                 ?>
                                 <option <?php echo  $selec?> value="<?php echo $row2 [0]?>"><?php echo $row2 [1]?></option>
                                 <?php  }
                                 ?>
                                 </select>
                           </div>
                           <div class="form-group">
                              ...
                              <input type="date" class="form-control"  value='<?php echo $row4['Release_date']?>' max="<?php echo date('Y-m-d')?>" name="reldate" required>
